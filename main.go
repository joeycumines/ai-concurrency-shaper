...
func run() error {
	...
	flag.Var(&transcodeCapabilities, "transcode-chat-capability", "chat-provider capability the transcoder may use (repeatable, comma/space separated; prefix ! to withdraw a default): developer_role, image_input, structured_outputs, parallel_tool_calls, stop_sequences, reasoning_effort, provider_reasoning_text, system_anywhere; the presets enable a maximally compatible core by default (parallel_tool_calls, provider_reasoning_text) — add reasoning_effort or developer_role for modern upstreams that accept them")
