...
func TestRepro_FirstChipOrphan(t *testing.T) {
	...
	// clamped the negative remainder to chipFloor, so the first chip was forced
	// below its natural width and rendered as an orphan on row 0.
	historical := NewModelForProviders([]ProviderMeta{
		{Name: "anthropic-eu-central", Concurrency: 4},
		{Name: "openai-prod-longname", Concurrency: 8},
		{Name: "acme-edge-provider", Concurrency: 12},
	})
	historical.width = 20
	historical.height = 24
	historical.active = 0
	historical.syncActive()
	oldRawBudget := historical.width - 2 - lipgloss.Width(historical.headerBody(true)) - 1
	oldBudget := max(oldRawBudget, historicalChipFloor)
	if oldBudget != historicalChipFloor {
		t.Fatalf("historical width=20 budget=%d (raw=%d), want floor=%d", oldBudget, oldRawBudget, historicalChipFloor)
	}
	firstLabel := " " + historical.providerLabel(0) + " "
	firstNatural := lipgloss.Width(historical.styles.chipActiveStyle.Render(firstLabel))
	if historicalChipFloor >= firstNatural {
		t.Fatalf("historical first chip natural width=%d, want greater than floor=%d", firstNatural, historicalChipFloor)
	}
	currentRows := historical.chipRowsLayout()
	if len(currentRows) == 0 || len(currentRows[0].providers) != 0 {
		t.Fatalf("current width=20 layout=%v, want an explicit empty row 0 after the historical orphan", currentRows)
	}
	...
	t.Logf("fleet header repro: row0Budget = width-2 - Width(headerBody(true)) -1, historicalChipFloor=%d, fullBudget=width-2", historicalChipFloor)
	t.Logf("expected flaw: for 3-long names body natural (~86) exceeds usable-4 up to width ~92, so row0Budget is degenerate historicalChipFloor=3 and first chip is truncated to \"  a\" alone on row0")
