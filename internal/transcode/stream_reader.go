...
type convertingReader struct {
	...
	// generatedFrameMax and generatedBatchMax bound the outbound frames and
	// terminal batches (the configured GeneratedSSEFrameBytes /
	// GeneratedSSEBatchBytes; review-z commit 3). generatedTotal bounds the
	// exchange-wide generated bytes. errorMessageMax bounds the error text
	// embedded in client-dialect error frames (ErrorMessageBytes).
	generatedFrameMax int
	generatedBatchMax int
	generatedTotal    int64
	errorMessageMax   int
}

// newConvertingReader wraps the source reader and converter. Zero generated
// limits select the package defaults.
// newConvertingReaderWithLimits wraps the source reader and converter with
// explicit generated-frame, generated-batch, and error-message bounds.
func newConvertingReaderWithLimits(
	source *SSEReader,
	conv frameConverter,
	generatedFrameMax int,
	generatedBatchMax int,
	...
	if generatedFrameMax <= 0 {
		generatedFrameMax = DefaultGeneratedSSEFrameBytes
	}
	if generatedBatchMax <= 0 {
		generatedBatchMax = DefaultGeneratedSSEBatchBytes
	}
	if errorMessageMax <= 0 {
		errorMessageMax = DefaultErrorMessageBytes
	}
	return &convertingReader{
		source:            source,
		conv:              conv,
		generatedFrameMax: generatedFrameMax,
		generatedBatchMax: generatedBatchMax,
		errorMessageMax:   errorMessageMax,
	}
}
...
func (r *convertingReader) appendErrorEvent(err error) error {
	// The configured error-message bound applies before the frame renders
	// (the converter's own package-default bound remains as defense).
	if r.errorMessageMax > 0 {
		err = truncateErrorText(err, r.errorMessageMax)
	}
	frame, ok := r.conv.ErrorEvent(err)
	if !ok {
		return nil
	}
	if err := writeFrameBytesBounded(&r.buf, frame, r.generatedFrameMax); err != nil {
		return err
	}
...
// upstream wire (UpstreamWireError) and internally inconsistent source data
// (SourceInconsistencyError) are always upstream; a StreamConversionError
...
// state.
// appendBatch commits a converted batch ATOMICALLY: the whole batch is first
// staged into a temporary buffer, validating every frame against
// generatedFrameMax and the staged total against generatedBatchMax; only a
// fully validated batch is copied into the live downstream buffer and only
// then does a terminal batch set sawTerminal. A success terminal can never
// escape before a later size failure, and a terminal batch exceeding
// generatedBatchMax is never partially delivered — the converter may then
// emit exactly one error event (review-z commit 3).
func (r *convertingReader) appendBatch(batch convertedBatch) error {
	var staged bytes.Buffer
	for _, event := range batch.Events {
		if err := writeFrameBytesBounded(&staged, event, r.generatedFrameMax); err != nil {
			return err
		}
	}
	if staged.Len() > r.generatedBatchMax {
...
// truncateErrorText wraps err so its Error() text is bounded to max bytes.
// The wrapped error keeps its identity (errors.As still sees the original)
// while every client-visible rendering is truncated.
func truncateErrorText(err error, max int) error {
	message := err.Error()
	if len(message) <= max {
		return err
	}
	...
	if max > 3 {
		return truncatedError{err: err, text: message[:max-3] + "…"}
	}
	return truncatedError{err: err, text: message[:max]}
}
